...
import org.activiti.crystalball.simulator.delegate.event.impl.FormPropertiesTaskCompleteTransformer;
 ...
 */
public class FormPropertiesTaskExecutionListener implements TaskListener {

    private final String typeToFind;
    protected final String typeToCreate;
    private final Collection<SimulationEvent> events;

    public FormPropertiesTaskExecutionListener(String typeToFind, String typeToCreate, Collection<SimulationEvent> events) {
    ...
    public void notify(DelegateTask delegateTask) {
        SimulationEvent eventToSimulate = findFormPropertiesTaskCompleteEvent(delegateTask);
        if (eventToSimulate != null) {
            Map<String, Object> properties = new HashMap<String, Object>();
            properties.put("taskId", delegateTask.getId());
            properties.put("variables", eventToSimulate.getProperty(FormPropertiesTaskCompleteTransformer.TASK_VARIABLES));
    ...
    private SimulationEvent findFormPropertiesTaskCompleteEvent(DelegateTask delegateTask) {
        if (delegateTask.hasVariable(StartReplayProcessEventHandler.PROCESS_INSTANCE_ID)) {
            ...
            for (SimulationEvent e : events) {
                if (typeToFind.equals(e.getType())
                        && toSimulateProcessInstanceId.equals(e.getProperty(FormPropertiesTaskCompleteTransformer.PROCESS_INSTANCE_ID))
                        && toSimulateTaskDefinitionKey.equals(e.getProperty(FormPropertiesTaskCompleteTransformer.TASK_DEFINITION_KEY)))
                    return e;
            }
        }
        return null;
